...
class M_emailrequest extends CI_Model {
...
// ------------------------------------------------------------------------------------------------------------
    ...
    public function _dashboard_query() {
        $rs = [
            "DAILY_REQUEST" => $this->_dashboard_daily_request(),
            "MONTHLY_REQUEST" => $this->_dashboard_monthly_request(),
            "YEARLY_REQUEST" => $this->_dashboard_yearly_request(),
            "TOTAL_REQUEST" => $this->_dashboard_total_request()
        ];

        return $rs;
    }

    public function _dashboard_daily_request() {
        date_default_timezone_set('Asia/Hong_Kong'); // Set your desired timezone 
        $this->db_1->from($this->tb_1);
        $this->db_1->group_start(); // START A GROUP FOR THE OR CONDITION
        $this->db_1->where('created_at >=', date('Y-m-d 00:00:00'));
        $this->db_1->where('created_at <=', date('Y-m-d 23:59:59'));
        $this->db_1->group_end(); // END THE GROUP

        $count = $this->db_1->count_all_results();
        return $count;
    }

    public function _dashboard_monthly_request() {
        $this->db_1->from($this->tb_1);
        $this->db_1->where('YEAR(created_at)', date('Y')); // Current year
        $this->db_1->where('MONTH(created_at)', date('m')); // Current month

        $count = $this->db_1->count_all_results(); // Get the count of matching rows
        return $count;
    }

    public function _dashboard_yearly_request() {
        $this->db_1->from($this->tb_1);
        $this->db_1->where('YEAR(created_at)', date('Y')); // Current year
    ...
    public function _dashboard_total_request() {
        $this->db_1->from($this->tb_1);
        $count = $this->db_1->count_all_results(); // Get the total count of rows
        return $count;
    }
